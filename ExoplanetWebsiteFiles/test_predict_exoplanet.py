...
import numpy as np
import re
from train_python_model import ExoplanetMLP
...
def create_light_curve_from_params(params):
    """
    Create a sophisticated synthetic light curve based on exoplanet parameters.
    This simulates what TESS would observe for the given parameters with realistic variations.
    """
    ...
    stellar_temp = params['stellar_temp']  # K
    tess_mag = params['tess_mag']  # TESS magnitude
    ...
    # Add stellar variability based on stellar temperature
    # Hotter stars tend to be more variable
    stellar_variability = 0.0001 * (stellar_temp / 6000)  # Scale with temperature
    stellar_period = 10.0  # days (typical stellar rotation period)
    flux += stellar_variability * np.sin(2 * np.pi * time / stellar_period)
    
    # Add transit signals with realistic ingress/egress
    if orbital_period > 0 and transit_duration > 0 and transit_depth > 0:
        ...
        for i in range(num_transits):
            ...
            # Add ingress/egress time (typically 10% of total duration)
            ingress_egress_time = transit_duration_days * 0.1
            ...
            # Create realistic transit profile with ingress/egress
            for j, t in enumerate(time[transit_mask]):
                if t < transit_start + ingress_egress_time:
                    # Ingress
                    progress = (t - transit_start) / ingress_egress_time
                    depth_factor = progress
                elif t > transit_end - ingress_egress_time:
                    # Egress
                    progress = (transit_end - t) / ingress_egress_time
                    depth_factor = progress
                else:
                    # Full transit
                    depth_factor = 1.0
                
                # Apply transit depth with realistic profile
                flux[transit_mask][j] -= transit_depth * depth_factor
    
    # Add realistic noise based on TESS magnitude
    # Brighter stars have lower noise
    base_noise = 0.0005  # Base noise level
    magnitude_noise_factor = 10 ** (0.4 * (tess_mag - 10))  # Noise increases with magnitude
    noise_level = base_noise * magnitude_noise_factor
    
    # Add correlated noise (realistic for space telescopes)
    correlated_noise = np.random.normal(0, noise_level * 0.3, len(flux))
    white_noise = np.random.normal(0, noise_level * 0.7, len(flux))
    
    # Apply noise
    flux += white_noise + correlated_noise
    
    # Add systematic effects
    # Long-term trend (detector drift)
    trend = 0.0001 * np.sin(2 * np.pi * time / observation_duration)
    flux += trend
    
    # Normalize but preserve relative variations
